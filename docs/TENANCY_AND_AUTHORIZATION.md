...
This document defines the conceptual security model. Milestone 3 implements the initial authentication boundary, tenant-membership model, centralized authorization, and database-level tenant isolation described below; see [Implementation Notes (Milestone 3)](#implementation-notes-milestone-3) for how each concept maps to running code. Milestone 4 extends the same tenant-scoped persistence and centralized-authorization pattern to the versioned evaluation domain; see [Implementation Notes (Milestone 4)](#implementation-notes-milestone-4). Workspace-scoped authorization, service-identity provisioning workflows, and administrative/emergency access paths remain conceptual until a later milestone implements them.
...
- **New actions**: `services/api/src/evalforge_api/domain/actions.py` extends `TenantAction` with `create_workspace` / `view_workspace`, `create_evaluation_target` / `view_evaluation_target`, `create_versioned_resource` / `view_versioned_resource`, `create_dataset` / `view_dataset`, `create_test_case`, `finalize_dataset_snapshot` / `view_dataset_snapshot`, and `create_artifact` / `view_artifact` — the same single deny-by-default permission table every Milestone 4 application service consults through `TenantContext.can()`, exactly as Milestone 3 established. Every role may view every evaluation-domain concept; `tenant_admin` and `evaluation_engineer` may create datasets, test cases, and finalize snapshots; `tenant_admin` and `developer` may create workspaces (workspace creation is `tenant_admin`-only, standing in for the not-yet-implemented workspace-administrator role), evaluation targets, and versioned configuration resources; `reviewer` and `read_only_observer` have no evaluation-domain mutation rights.
- **Tenant-scoped storage and persistence**: every Milestone 4 table (`workspaces`, `evaluation_targets`, `versioned_resources`, `versioned_resource_versions`, `datasets`, `test_cases`, `test_case_versions`, `dataset_snapshots`, `dataset_snapshot_items`, `artifacts`, `artifact_versions`) carries an explicit `tenant_id` column, PostgreSQL row-level security (`ENABLE`/`FORCE ROW LEVEL SECURITY`) keyed on the same transaction-local `app.current_tenant_id` session setting Milestone 3 introduced, and a `SELECT`/`INSERT`-only grant to `evalforge_app` (`SELECT`/`INSERT`/`UPDATE` on `dataset_snapshots` only, for the draft-to-finalized transition — see [Domain Model](DOMAIN_MODEL.md#implementation-notes-milestone-4)). No table grants `DELETE`.
- **Preventing accidental lookup by global ID**: every child table's foreign key to its parent is a composite `(id, tenant_id)` reference (`evalforge_api.adapters.rls_session.set_tenant_session` populates the session setting every repository method requires), so a row can only reference a parent that belongs to the *same* tenant — a cross-tenant reference is rejected by a `ForeignKeyViolationError` at the database level even if application code or RLS were bypassed.
- **Artifact object storage**: `evalforge_api.adapters.artifact_object_storage.S3ArtifactObjectStorage` never derives a storage key from caller input; `evalforge_api.application.artifact_service` constructs every key from server-verified `tenant_id` plus the computed content hash (`tenants/{tenant_id}/artifacts/{artifact_id}/versions/{version_number}-{content_hash}`), so no request can influence where bytes land or reach another tenant's object.

Related documents: [Domain Model](DOMAIN_MODEL.md), [Trust Boundaries](TRUST_BOUNDARIES.md), [Security Baseline](SECURITY_BASELINE.md), ADR [0004](adr/0004-tenant-isolation-and-data-ownership.md), the [Milestone 3 Completion Report](MILESTONE_3_COMPLETION_REPORT.md), and the [Milestone 4 Completion Report](MILESTONE_4_COMPLETION_REPORT.md).
